...
// File Name: file.c
// Create Time: Mon 24 Jan 2022 05:20:51 PM CST
// This is ... 
...
// 将全局描述符下标(数组file_table的下标)安装到进程或线程自己的文件描述符数组fd_table中,成功返回下标,失败返回-1
...
int32_t file_create(struct dir* parent_dir, char* filename, uint8_t flag)
{
    ...
    // 返回的是file_table数组的下标
    fd_idx = get_free_slot_in_global();
    if (fd_idx == -1)
    {
        printk("exceed max open files\n");
        rollback_step = 2;
...
rollback:
    ...
    {
        case 3: // 失败时,将file_table中的相应位清空
            memset(&file_table[fd_idx], 0, sizeof(struct file));
        case 2:
            sys_free(new_file_inode);
        case 1: // 如果新文件的i结点创建失败,之前位图中分配的inode_no也要恢复
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) 
{
    ...
    // 开始将文件所有块地址收集到all_blocks,(系统中块大小等于扇区大小)后面都统一在all_blocks中获取写入扇区地址
    if (add_blocks == 0) 
    { 
...
int32_t file_read(struct file* file, void* buf, uint32_t count)
{
    ...
    uint32_t block_read_start_idx = file->fd_pos / BLOCK_SIZE;              // 数据所在起始块的地址
    uint32_t block_read_end_idx = (file->fd_pos + size) / BLOCK_SIZE;       // 数据所在终止块的地址
    ...
    else  // 若要读多个块
    {
        ...
        // 第二种情况: 待读入的数据跨越直接块和间接块两类,先将直接块地址写入all_blocks
