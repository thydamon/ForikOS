...
// File Name: fs.c
// Create Time: Sun 23 Jan 2022 01:18:59 PM CST
// This is ... 
...
static void partition_format(struct partition* part)
{
    ...
    // 空闲块位图占用了一部分空闲扇区, 因此现在真正的空闲块数得把block_bitmap_sets从free_sets中减去,其结果也是位图中位的个数
    uint32_t block_bitmap_bit_len = free_sects - block_bitmap_sects;
    // block_bitmap_bit_len重新除以BITS_PER_SECTOR,这便是空闲块位图最终占用的扇区数block_bitmap_sects
    ...
    // 元信息最终是要写入硬盘的,但数据源是在内存中,像超级块本身是1扇区大小,我们是用局部变量声明它的,栈还能将就对付
    // 空闲位图、inode数组位图等占用的扇区数较大,所以这里不便用局部变量来保存它们了,找出数据量最大的元信息,用其尺寸作为申请的内存大小
...
int32_t path_depth_cnt(char* pathname)
{
    ...
    // 解析路径,从中拆分出各级名称
    p = path_parse(p, name);  // path_parse逐级返回目录名
    while (name[0])
    {
        depth++;  // 按目录递增
