...
// File Name: inode.c
// Create Time: Mon 24 Jan 2022 12:20:16 PM CST
// This is ... 
...
static void inode_locate(struct partition* part, uint32_t inode_no, struct inode_position* inode_pos)
{
    ...
    // 从分区超级块的inode_table_Iba中获取inode_table的扇区地址,存入同名变量
    uint32_t inode_table_lba = part->sb->inode_table_lba;

    uint32_t inode_size = sizeof(struct inode);
    // 获取编号为inode_no的inode的位置
...
{
    // 磁盘是一种低速设备,为减少频繁访问磁盘,在内存中为各分区创建了inode队列
    ...
    // 需要将inode置于内核空间,故需要临时,将cur_pbc->pgdir置为NULL
...
void inode_release(struct partition* part, uint32_t inode_no)
{
    ...
    // 以下inode_delete是调试用的
    // 此函数会在inode_table中将此inode清0,但实际上是不需要的,inode分配是由inode位图控制的,硬盘上的数据不需要清0,可以直接覆盖
