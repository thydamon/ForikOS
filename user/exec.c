...
// File Name: user/exec.c
// Create Time: Mon 31 Jan 2022 03:04:06 PM CST
// This is ... 
...
static bool segment_load(int32_t fd, uint32_t offset, uint32_t filesz, uint32_t vaddr)
{
    ...
    while (page_idx < occupy_pages)
    {
        ...
        // 如果pde不存在,或者pte不存在就分配内存,pde的判断要在pte之前,否则pde若不存在会导致
        // 判断pte时缺页异常
...
static int32_t load(const char* pathname)
{
    ...
    // 校验elf头
    if (memcmp(elf_header.e_ident, "\177ELF\1\1\1", 7) || elf_header.e_type != 2 || elf_header.e_machine != 3 || elf_header.e_version != 1 || elf_header.e_phentsize != sizeof(struct Elf32_Phdr)) 
    {
        ret = -1;
        goto done;
    }
