...
// File Name: ide.c
// Create Time: Sat 22 Jan 2022 08:11:31 PM CST
// This is ... 
...
{
    // 利用device寄存器中的dev位,该位为0表示是通道中的主盘,为1表示是通道的从盘
...
void ide_read(struct disk* hd, uint32_t lba, void* buf, uint32_t sec_cnt)
{
    ...
    {
        // 为读写扇区数端口0x1f2及0x172是8位寄存器,故每次读
        // 写最多是255个扇区,(当写入端口值为0时,则写入256个扇区)
        // 当读写的端口数超过256时,必须拆分成多次读写操作。
