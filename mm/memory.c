...
// 返回arena中第idx个内存块的地址,arena->mem_block
static struct mem_block* arena2block(struct arena* a, uint32_t idx)
{
    return (struct mem_block*)((uint32_t)a + sizeof(struct arena) + idx * a->desc->block_size);
}

// 返回内存块b所在的arena地址,mem_block->arena
...
void* sys_malloc(uint32_t size)
{
    ...
    if (size > 1024)
    {
        ...
        if (a != NULL)
        {
            ...
            // 初始化内存块
            a->desc = NULL;   // 大内存块desc赋值为NULL
            a->cnt = page_cnt; 
            a->large = true;  // 大内存块large赋值为true
	        lock_release(&mem_pool->lock);

            return (void*)(a+1);  // a+1用于跳过元信息
        }
        else
        {
	        lock_release(&mem_pool->lock);
    ...
    else 
    {
        ...
        // 从7种规格的内存块中找到大小合适的内存块
        for (desc_idx = 0; desc_idx < DESC_CNT; desc_idx++)
        {
            if (size <= descs[desc_idx].block_size)  // 从小到大,找到后退出
            {
                break;
            }
        }
...
static void vaddr_remove(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt)
{
    ...
    {
        struct task_struct* cur_thread = running_thread();
        bit_idx_start = (vaddr - cur_thread->user_vaddr.vaddr_start) / PG_SIZE;
        while(cnt < pg_cnt)
        {
